        ...
        if ($status_location == 1 && $radius > $lok_kantor->radius_cabang) {
            ...
            if ($cek > 0) {
                if ($jam_pulang < $jamkerja_pulang) {
                    ...
                    echo "error|Anda Sudah Pernah Absen Pulang ! |out";
                } else {
                    $data_pulang = [
                        'jam_out' => $jam,
                        'foto_out' => $fileName,
                ...
                if ($jam < $jamkerja->awal_jam_masuk) {
                    ...
                    if ($simpan) {
                        ...
                        curl_setopt_array($curl, array(
                            ...
                            CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                            CURLOPT_CUSTOMREQUEST => 'POST',
                            CURLOPT_POSTFIELDS => array('message' => 'Terima kasih Sudah Melakukan Absen Masuk, Anda Melakukan Absen Pada Jam ' . $jam, 'number' => $no_hp, 'file_dikirim' => ''),
                        ));
